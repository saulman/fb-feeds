...
angular.module('rssApp')
  ...
  .controller('ScrapCtrl', ['$scope','$state','$http','$location','$stateParams','FBPageFeed','$timeout','$cookieStore','$rootScope', function ($scope, $state, $http, $location, $stateParams,FBPageFeed,$timeout,$cookieStore,$rootScope) {
   

    $scope.someval = true;

    if ($scope.someval.length > 0) $scope.someval = false;
   ...
   $scope.loading = false;
    $scope.syncFeed = function(maxLimit, nextURL) {
        ...
        if ($scope.feed != null){
          ...
          FB_lastChar = fbURL.charAt(fbURL.length - 1);
          if (FB_lastChar == "/") {
          ...
          return $http.get(nextURL)
            .success(function(fbNextFeed) {
              ...
              if(fbNextFeed.paging){
               ...
               angular.forEach(fbNextFeed.data, function(value, key){
                  checkPost(value.id)
